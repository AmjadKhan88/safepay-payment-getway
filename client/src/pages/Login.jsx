...
import axios from 'axios'
   ...
   const Login = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        const apiUrl = state === "login" ? "/api/users/login" : "/api/users/signup";
        try {
          const {data} = await axios.post(`http://localhost:5000${apiUrl}`,formData);

          
            data ? setUser(data.user) : navigate('/login')
            navigate("/")
        } catch (error) {
    ...
    return (
            <form onSubmit={handleSubmit} className="sm:w-[350px] mx-auto mt-10 sm:mt-15 md:mt20 w-full text-center border border-gray-300/60 rounded-2xl px-8 bg-white">
